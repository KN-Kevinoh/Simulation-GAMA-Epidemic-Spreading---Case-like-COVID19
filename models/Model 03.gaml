 ...
 * 
 * @add scenario to fight against disease
 * @simulation an statistics
 * 
 */
 global {
	...
	int distance_neighbors <- 1 #m;
	float contract_disease_probability <- 0.2;
...
species hospital parent:building skills: [fipa]{
	list<hospital> Hospitals <- [] update: (building where (hospitals_list contains each.type_building));
	
	reflex send_infos when : (time=1) {
		write '(Time ' + time + '): ' + name + ' sends a cfp  message to all hospitals';
		do start_conversation to: list(Hospitals) protocol: 'fipa-contract-net' performative: 'cfp' contents: ['Go swimming'] ;
	}
	 
	reflex receive_infos when: !empty(proposes){
		...
		loop p over: proposes {
			write '\t' + name + ' receives a propose message from ' + agent(p.sender).name + ' with content ' + p.contents ;
				...
				do accept_proposal message: p contents: ['Very helpful. Thanks!'] ;
...
experiment Model03 type: gui {
	...
	parameter "Move probability" var:move_probability min:0 max:1 category: "People";
	parameter "Avoid at distance" var: distance_neighbors min:1 max:10 category: "doctor";
	parameter "Number doctor by hospital" var: nb_doctors  min:2 max:50 category: "doctor";
	parameter "Threshold people for quarantine" var: threshold_for_quarantine min:5 max:200 category: "doctor";
	parameter "infection rate" var: contract_disease_probability min:0 max:1 category: "doctor";
	output {
		...
		display peopleDistribution{
			...
			chart "people with age distribution" type: histogram{
				data "age least than 20" value: people count( each.age <= 20 ) color:#orange;
				data "age between 19 and 40" value: people count( 20 < each.age and each.age < 40 ) color:#blue;
				data "age between 40 and 60" value: people count( 40 < each.age and each.age <= 60 )  color:#brown;
				data "age between 60 and 80" value: people count( 40 < each.age and each.age <= 80 ) color:#olive;
				data "age over 80"   value: people count( 80 < each.age and each.age <= 100 ) color:#yellow;
				data "doctors between 25 and 95"   value: doctor count( 25 < each.age and each.age <= 95 ) color:#violet;
			}
		}
		...
		display BuildingsDistribution{
			...
			chart "buildings with type distribution" type: pie{
				data "Living" value: length(building where  (residences_list contains each.type_building))  color:#brown;
				data "Works" value: length(building where  (works_list contains each.type_building))  color:#blue;
				data "Schools" value:length(building where  (schools_list contains each.type_building)) color:#yellow;
				data "Hospitals fighting disease" value:length(hospital) color:#green;
				data "Hobbies" value:length(building where  (hobbies_list contains each.type_building)) color:#orange;
				data "Commercials" value:length(building where  (commercials_list contains each.type_building))  color:#olive;
			}
		}
		
		
		 display overview refresh:every(10 #cycles) {
	        chart "Disease spreading" type: series {
		        data "Susceptible" value: people count(not(each.is_infected)) color: #green;
		        data "Infected" value: people count(each.is_infected) color: #red;
		        data "Dead" value: people count(each.color=#black) color: #red;
		...
		monitor "people infected" value: people count(each.is_infected) color:#orange ;
		monitor "People infected level 1" value: people  count(each.color = #violet) color:#violet;
		monitor "People infected level 2" value: people  count(each.color = #red) color:#red;
		monitor "people died among the infected" value: people count(each.color=#black) color:#black;
		monitor "people infected least than 30" value: people count(each.is_infected and each.age < 30 ) color:#blue ;
		monitor "people infected between 30 and 70" value: people count(each.is_infected and 29 < each.age and each.age < 70) color:#blue ;
		monitor "people infected over 70" value: people count(each.is_infected and 69 < each.age and each.age < 1001) color:#blue ;
